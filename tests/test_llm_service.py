...
pytestmark = pytest.mark.anyio  # run async tests with anyio/asyncio


# -------------------- generate_learning_path_logic --------------------

async def _mk_learning_path(topics_count=10) -> LearningPathResponse:
    return LearningPathResponse(domain_name="Python Programming", topics=[f"Topic {i}" for i in range(1, topics_count+1)])
...
async def test_generate_learning_path_with_assessment_invokes_analyzer_and_passes_profile(mocker, assessment_answer):
    ...
    def lp_side_effect(payload_json: str):
        ...
        return SimpleNamespace(output=LearningPathResponse(domain_name="Python Programming", topics=[f"Topic {i}" for i in range(1, 11)]))
...
async def test_generate_learning_path_without_performance_uses_fallback_and_skips_analyzer(mocker):
    ...
    def lp_side_effect(payload_json: str):
        ...
        return SimpleNamespace(output=LearningPathResponse(domain_name="Data Science", topics=[f"Unit {i}" for i in range(1, 11)]))
...
async def test_generate_learning_path_analyzer_error_is_tolerated(mocker, assessment_answer):
    ...
    result = await llm_service.generate_learning_path_logic("Python Programming", user_id=1, user_topic_performance_data=perf)
    assert isinstance(result, LearningPathResponse)
...
async def test_generate_insights_happy_path_sequential_calls(mocker, assessment_answer, long_explanation):
    ...
    mocker.patch.object(llm_service._content_adaptation_planner_agent, "run", new=AsyncMock(side_effect=planner_side_effect))
...
async def test_generate_insights_fallback_plan_when_no_perf_data(mocker, long_explanation):
    # Analyzer should not be called
    mocker.patch.object(llm_service._user_data_analyzer_agent, "run", new=AsyncMock(side_effect=AssertionError("No analyzer")))
