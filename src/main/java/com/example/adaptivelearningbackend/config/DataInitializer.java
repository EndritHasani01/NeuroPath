...
public class DataInitializer implements CommandLineRunner {
        ...
        private void ensureRolesExist() {
                roleRepo.findByName("ROLE_USER").orElseGet(() -> {
                        logger.info("Creating default ROLE_USER role");
                        return roleRepo.save(RoleEntity.builder().name("ROLE_USER").build());
                });

                roleRepo.findByName("ROLE_ADMIN").orElseGet(() -> {
                        logger.info("Creating default ROLE_ADMIN role");
                        return roleRepo.save(RoleEntity.builder().name("ROLE_ADMIN").build());
                });
        }
}
